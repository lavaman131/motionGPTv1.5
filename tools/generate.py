...
import mgpt.data_loaders.humanml.utils.paramUtil as paramUtil
from mgpt.data_loaders.humanml.utils.plot_script import plot_3d_motion_mix
import json
...
def main():
    ...
    if out_path == "":  # if unspecified, save in the same folder as the model
        out_path = os.path.join(
            os.path.dirname(args.model_path), "{}_s{}".format(niter, args.seed)
        )
        if args.instructions_file != "":
            out_path += "_" + os.path.basename(args.instructions_file).replace(
                ".json", ""
            ).replace(" ", "_").replace(".", "")
