...
def babel_collate(batch):
    from mgpt.data_loaders.amass.tools import collate_pairs_and_text
...
def collate(batch):
    ...
    maskbatchTensor = (
        lengths_to_mask(lenbatchTensor, max_seq_len).unsqueeze(1).unsqueeze(1)
    ...
    if "all_texts" in notnone_batches[0] and "all_lengths" in notnone_batches[0]:
        ...
        conditions_mask = torch.zeros(
            (num_max_segs, max_seq_len, batch_size), device=device, dtype=torch.bool
        )
        for i, c_all_lengths in enumerate(all_lengths):
            s = 0
            for j, length in enumerate(c_all_lengths):
                conditions_mask[j, s : s + length, i] = (
                    True  # all batch elements have the same instructions
                )
                s += length
        cond["y"].update({"conditions_mask": conditions_mask})

        pos_pe_abs = torch.zeros(
        ...
        for i, c_all_lengths in enumerate(all_lengths):
            ...
            for length in c_all_lengths:
                pos_pe_abs[i, s : s + length] = torch.arange(
                    length, device=device, dtype=torch.float32
                )
                pe_bias[i, s : s + length, s : s + length] = (
                    0  # only attend to the segment for the absolute modeling part of the schedule
                )
                s += length

        cond["y"]["pe_bias"] = (
            pe_bias  # in MDM forward, it is selected according to the mixed schedule if active
        )
        ...
        if "text_embeddings" in notnone_batches[0]:
            ...
            text_embeddings = torch.zeros(
                (max_len, batch_size, 512), device=device, dtype=torch.float32
            )
            for i, texts in enumerate(texts_list):
                text_embeddings[: len(texts), i, :] = notnone_batches[i][
                    "text_embeddings"
                ][: len(texts)]
        ...
        cond["y"].update(
            {"text_embeddings": text_embeddings.to(databatchTensor.device)}
        )
...
def t2m_collate(batch):
    ...
    adapted_batch = [
        {
            "inp": torch.tensor(b[4].transpose((1, 0))).float().unsqueeze(1)
            if len(b[4].shape) == 2
            else torch.tensor(
                b[4].transpose((1, 2, 0))
            ).float(),  # [seqlen, J] -> [J, 1, seqlen]
            "text": b[2],  # b[0]['caption']
            "tokens": b[6],
            "lengths": b[5],
        }
    ...
    if len(batch[0]) >= 9 and not isinstance(
        batch[0][8], list
    ):  # then load precomputed text embeddings
