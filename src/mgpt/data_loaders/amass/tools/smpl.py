...
from mgpt.data_loaders.amass.tools_teach import geometry
from mgpt.data_loaders.amass.tools_teach.easyconvert import axis_angle_to
from mgpt.data_loaders.amass.transforms.smpl import RotTransDatastruct
...
def canonicalize_smplh(poses: Tensor, trans: Optional[Tensor] = None):
    ...
    if trans is not None:
        ...
        trans = torch.cat(
            (torch.zeros(bs, 1, 3, device=vel.device), torch.cumsum(vel, 1)), 1
        )
        return xc, trans
    else:
        return xc
