...
from mgpt.constants.amass.labels import fix_spell
...
def separate_actions(pair: Tuple[Tuple]):
    if len(pair) == 3:
        ...
        final_pair = [(pair[0][0], pair[0][1]), (pair[1][0], pair[1][1])]

    return final_pair


def timeline_overlaps(
    arr1: Tuple, arr2: List[Tuple]
...
def extract_frame_labels(babel_labels, fps, seqlen, datatype):
    ...
    if is_valid:
        if babel_labels["frame_ann"] is None:
            ...
            if (
                datatype == "pairs"
                or datatype == "pairs_only"
                or datatype == "separate_pairs"
            ):
                import itertools
                ...
                seg_acts_for_pairs = [
                    a for a in seg_acts if a not in EXCLUDED_ACTIONS_WO_TR
                ]
                idx_to_keep = [
                    i for i, a in enumerate(seg_acts) if a not in EXCLUDED_ACTIONS_WO_TR
                ]
                seg_ids_for_pairs = [
                    s for i, s in enumerate(seg_ids) if i in idx_to_keep
                ]
                    ...
                    if seg2act[seg_] == "transition":
                        ...
                        if not seg_[0] == 0:
                            if ov_seg["before"] and ov_seg["after"]:
                                cur_seg_pairs = list(
                                    itertools.product(ov_seg["before"], ov_seg["after"])
                                )
                                cur_act_pairs = [
                                    (seg2act[x], seg2act[y]) for x, y in cur_seg_pairs
                                ]
                                if not datatype == "separate_pairs":
                                    cur_seg_pairs = [
                                        (
                                            min(min(a), min(b), min(seg_)),
                                            max(max(a), max(b), max(seg_)),
                                        )
                                        for a, b in cur_seg_pairs
                                    ]
                                else:
                                    cur_seg_pairs = [
                                        tuple(sorted(p, key=lambda item: item[0]))
                                        for p in cur_seg_pairs
                                    ]
                                    cur_seg_pairs = [
                                        (a, seg_, b) for a, b in cur_seg_pairs
                                    ]
                        ...
                        ov_seg["before"] = [
                            x for x in ov_seg["before"] if seg2act[x] != "transition"
                        ]
                        ov_seg["after"] = [
                            x for x in ov_seg["after"] if seg2act[x] != "transition"
                        ]
                        if ov_seg["before"]:
                            cur_seg_pairs_bef = list(
                                itertools.product(ov_seg["before"], [seg_])
                            )
                        if ov_seg["after"]:
                            cur_seg_pairs_af = list(
                                itertools.product([seg_], ov_seg["after"])
                            )
                        ...
                        if not datatype == "separate_pairs":
                            ...
                            cur_seg_pairs = [
                                (min(min(a), min(b)), max(max(a), max(b)))
                                for a, b in cur_seg_pairs
                            ]
                        else:
                            # just to be sure
...
def get_all_seq_durs(duration, max_len, min_len, crop_samples):
    if crop_samples:
        ...
        dur1_t = (
            dur1 + dur_tr
            if ((dur1 >= min_len) and ((dur1 + dur_tr) <= (max_len + 4)))
            else None
        )
        dur2_t = (
            dur2 + dur_tr
            if ((dur2 >= min_len) and ((dur2 + dur_tr) <= (max_len + 4)))
            else None
        )
        durations = {
...
class BABEL(Dataset):
        ...
        **kwargs,
        ...
        if tiny:
            ...
            self.babel_annots = read_json(
                Path(datapath) / f"../babel-teach/{split}.json"
            )
        else:
        ...
        num_not_kit = 0
        self.sep_to_4 = True
        assert self.sep_to_4

        precomputed_folder = "./dataset/babel/tmp"
        suffix = f"{sampler.min_len}_{sampler.max_len}_{crop_samples}"
        if not os.path.exists(precomputed_folder) or not os.path.exists(
            os.path.join(precomputed_folder, f"{split}_motion_data_{suffix}.pkl")
        ):
            if progress_bar:
            ...
            for i, sample in enumerator:
                ...
                seg_ids, seg_acts, valid = extract_frame_labels(
                    self.babel_annots[babel_id],
                    fps=last_framerate,
                ...
                for index, seg in enumerate(seg_ids):
                    ...
                    if self.dtype == "separate_pairs":
                        ...
                        if self.sep_to_4:
                            ...
                            dur_dict = get_all_seq_durs(
                                duration, sampler.max_len, sampler.min_len, crop_samples
                            )
                            dur1, bias_dur1 = dur_dict["dur1"], dur_dict["bias_dur1"]
                            dur2, bias_dur2 = dur_dict["dur2"], dur_dict["bias_dur2"]
                            dur1_t, bias_dur1_t = (
                                dur_dict["dur1_t"],
                                dur_dict["bias_dur1_t"],
                            )
                            dur2_t, bias_dur2_t = (
                                dur_dict["dur2_t"],
                                dur_dict["bias_dur2_t"],
                            )
                            if (
                                dur1 is not None
                            ):  # we retrieve data needed for the first separate sequence if length within limits
                                frames_0_valid = True
                                frames_0 = np.arange(fpair[0][0], fpair[0][1] + 1)[
                                    bias_dur1 : bias_dur1 + dur1
                                ]
                                is_transition_0 = torch.zeros(dur_1)[
                                    bias_dur1 : bias_dur1 + dur1
                                ]
                                if (dur_tr_valid) and (dur1_t is not None):
                                    frames_0_T = np.arange(
                                        fpair[0][0], fpair[1][1] + 1
                                    )[
                                        bias_dur1_t : bias_dur1_t + dur1_t
                                    ]  # ---> WITH TRANSITION INCLUDED
                                    is_transition_0_T = torch.concatenate(
                                        (torch.zeros(dur_1), torch.ones(dur_tr))
                                    )[bias_dur1_t : bias_dur1_t + dur1_t]
                            if (
                                dur2 is not None
                            ):  # we retrieve data needed for the second separate sequence if length within limits
                                frames_1_valid = True
                                frames_1 = np.arange(fpair[-1][0], fpair[-1][1] + 1)[
                                    bias_dur2 : bias_dur2 + dur2
                                ]
                                is_transition_1 = torch.zeros(dur_2)[
                                    bias_dur2 : bias_dur2 + dur2
                                ]
                                if dur_tr_valid and (dur2_t is not None):
                                    frames_1_T = np.arange(
                                        fpair[1][0], fpair[-1][1] + 1
                                    )[
                                        bias_dur2_t : bias_dur2_t + dur2_t
                                    ]  # ---> WITH TRANSITION INCLUDED
                                    is_transition_1_T = torch.concatenate(
                                        (torch.ones(dur_tr), torch.zeros(dur_2))
                                    )[bias_dur2_t : bias_dur2_t + dur2_t]
                        else:
                        ...
                        if frames_0_valid:
                            smpl_data_0 = {
                                "poses": torch.from_numpy(
                                    sample["poses"][frames_0]
                                ).float(),
                                "trans": torch.from_numpy(
                                    sample["trans"][frames_0]
                                ).float(),
                            }
                            if dur_tr_valid and (dur1_t is not None):
                                smpl_data_0_T = {
                                    "poses": torch.from_numpy(
                                        sample["poses"][frames_0_T]
                                    ).float(),
                                    "trans": torch.from_numpy(
                                        sample["trans"][frames_0_T]
                                    ).float(),
                                }
                        if frames_1_valid:
                            smpl_data_1 = {
                                "poses": torch.from_numpy(
                                    sample["poses"][frames_1]
                                ).float(),
                                "trans": torch.from_numpy(
                                    sample["trans"][frames_1]
                                ).float(),
                            }
                            if dur_tr_valid and (dur2_t is not None):
                                smpl_data_1_T = {
                                    "poses": torch.from_numpy(
                                        sample["poses"][frames_1_T]
                                    ).float(),
                                    "trans": torch.from_numpy(
                                        sample["trans"][frames_1_T]
                                    ).float(),
                                }
                    else:
                        smpl_data = {
                    ...
                    if not self.dtype == "separate_pairs":
                        ...
                        if not self.sep_to_4:  # do not accept by duration
                            dur1, dur_tr, dur2 = duration
                            if not (
                                self.sampler.accept(dur1)
                                and self.sampler.accept(dur2)
                                and self.sampler.accept(dur1 + dur_tr)
                                and self.sampler.accept(dur2 + dur_tr)
                            ):
                                num_bad_short += 1
                                continue
                    valid_data_len += 1
                    ...
                    from mgpt.data_loaders.amass.tools.smpl import (
                        smpl_data_to_matrix_and_trans,
                    )

                    # here we take smpl data in poses/trans and move to rots/trans
                    if self.sep_to_4:
                        if frames_0_valid:
                            smpl_data_0 = smpl_data_to_matrix_and_trans(
                                smpl_data_0, nohands=True
                            )
                            if dur_tr_valid and (dur1_t is not None):
                                smpl_data_0_T = smpl_data_to_matrix_and_trans(
                                    smpl_data_0_T, nohands=True
                                )
                        if frames_1_valid:
                            smpl_data_1 = smpl_data_to_matrix_and_trans(
                                smpl_data_1, nohands=True
                            )
                            if dur_tr_valid and (dur2_t is not None):
                                smpl_data_1_T = smpl_data_to_matrix_and_trans(
                                    smpl_data_1_T, nohands=True
                                )
                    else:
                        smpl_data = smpl_data_to_matrix_and_trans(
                            smpl_data, nohands=True
                        )
                    ...
                    if mode == "train":
                        ...
                        if load_with_rot:
                            if self.sep_to_4:
                                if frames_0_valid:
                                    features_0 = self.transforms.rots2rfeats(
                                        smpl_data_0
                                    )
                                    if dur_tr_valid and (dur1_t is not None):
                                        features_0_T = self.transforms.rots2rfeats(
                                            smpl_data_0_T
                                        )
                                if frames_1_valid:
                                    features_1 = self.transforms.rots2rfeats(
                                        smpl_data_1
                                    )
                                    if dur_tr_valid and (dur2_t is not None):
                                        features_1_T = self.transforms.rots2rfeats(
                                            smpl_data_1_T
                                        )
                            else:
                    ...
                    if self.dtype == "separate_pairs":
                        if self.sep_to_4:
                            if frames_0_valid:
                                texts_data[f"{babel_id}-{index}-{1}"] = seg_acts[index][
                                    0
                                ]
                                durations[f"{babel_id}-{index}-{1}"] = dur1
                                transitions_data[f"{babel_id}-{index}-{1}"] = (
                                    is_transition_0
                                )
                                if dur_tr_valid and (dur1_t is not None):
                                    texts_data[f"{babel_id}-{index}-{1_1}"] = seg_acts[
                                        index
                                    ][0]
                                    durations[f"{babel_id}-{index}-{1_1}"] = dur1_t
                                    transitions_data[f"{babel_id}-{index}-{1_1}"] = (
                                        is_transition_0_T
                                    )
                            if frames_1_valid:
                                texts_data[f"{babel_id}-{index}-{2}"] = seg_acts[index][
                                    1
                                ]
                                durations[f"{babel_id}-{index}-{2}"] = dur2
                                transitions_data[f"{babel_id}-{index}-{2}"] = (
                                    is_transition_1
                                )
                                if dur_tr_valid and (dur2_t is not None):
                                    texts_data[f"{babel_id}-{index}-{2_1}"] = seg_acts[
                                        index
                                    ][1]
                                    durations[f"{babel_id}-{index}-{2_1}"] = dur2_t
                                    transitions_data[f"{babel_id}-{index}-{2_1}"] = (
                                        is_transition_1_T
                                    )
                        else:
                    ...
                    if mode == "train":
                        if self.sep_to_4:
                            if frames_0_valid:
                                ...
                                if dur_tr_valid and (dur1_t is not None):
                                    motion_data[f"{babel_id}-{index}-{1_1}"] = (
                                        features_0_T
                                    )
                            ...
                            if frames_1_valid:
                                ...
                                if dur_tr_valid and (dur2_t is not None):
                                    motion_data[f"{babel_id}-{index}-{2_1}"] = (
                                        features_1_T
                                    )
                        else:
            ...
            if split != "test" and not tiny:
                ...
                percentage = (
                    100 * num_bad_actions / (total + num_bad_short + num_bad_actions)
                )
                logger.info(
                    f"{percentage:.4}% of the sequence which are rejected by the sampler, because of the excluded actions."
                )
                percentage = (
                    100 * num_bad_short / (total + num_bad_short + num_bad_actions)
                )
                logger.info(
            ...
            if self.parse_tokens:
                ...
                with open(
                    os.path.join(precomputed_folder, f"{split}_tokens_data.pkl"), "wb"
                ) as f:
            ...
            with open(
                os.path.join(precomputed_folder, f"{split}_texts_data_{suffix}.pkl"),
                "wb",
            ) as f:
                pickle.dump(texts_data, f)
            with open(
                os.path.join(precomputed_folder, f"{split}_durations_{suffix}.pkl"),
                "wb",
            ) as f:
                pickle.dump(durations, f)
            with open(
                os.path.join(
                    precomputed_folder, f"{split}_transitions_data_{suffix}.pkl"
                ),
                "wb",
            ) as f:
            ...
            if split == "train":
                ...
                from mgpt.data_loaders.amass.transforms.smpl import SlimSMPLTransform
                ...
                for keyid, motion in tqdm(motion_data.items()):
                    motion_xyz = self.transform.SlimDatastruct(
                        features=motion.unsqueeze(0)
                    ).joints.permute(0, 2, 3, 1)
                    motions_xyz[keyid] = motion_xyz.squeeze()

                with open(
                    os.path.join(
                        precomputed_folder, f"{split}_motions_xyz_{suffix}.pkl"
                    ),
                    "wb",
                ) as f:
            ...
            if self.parse_tokens and os.path.exists(
                os.path.join(precomputed_folder, f"{split}_tokens_data.pkl")
            ):
                with open(
                    os.path.join(
                        precomputed_folder, f"{split}_tokens_data_{suffix}.pkl"
                    ),
                    "rb",
                ) as f:
    ...
    def load_keyid(self, keyid, mode="train"):
        ...
        if mode == "train":
            ...
            if self.dtype == "separate_pairs":
                if self.sep_to_4:
                    ...
                    element = {
                        ...
                        "keyid": keyid,
                        "text": text,
                        "features_xyz": self.motions_xyz[keyid]
                        if self.split == "train"
                        else torch.zeros(1),
                    }
                else:
                    features = self.motion_data[keyid]
                    length_0, length_transition, length_1 = (
                        self._num_frames_in_sequence[keyid]
                    )

                    features_0 = features[:length_0]  # lenght_0
                    features_1_with_transition = features[
                        length_0:
                    ]  # lenght_1 + length_transition
                    features_1 = features[(length_0 + length_transition) :]  # lenght_1

                    features_0_with_transition = features[
                        : (length_0 + length_transition)
                    ]
            ...
            raise ValueError(
                "mdm project - you should never use mode other than train in our scope"
            )
        return element

    def load_seqid(self, seqid):
        segs_keyids = [
            keyid for keyid in self._split_index if keyid.split("-")[0] == seqid
        ]
        segs_keyids = sorted(
            [(e.split("-")[0], int(e.split("-")[1])) for e in segs_keyids],
            key=lambda x: x[1],
        )
        ...
        if len(segs_keyids) == 1:
            ...
            while True:
                ...
                candidate_next = [
                    i
                    for i in segs_keyids[(segs_keyids.index(current) + 1) :]
