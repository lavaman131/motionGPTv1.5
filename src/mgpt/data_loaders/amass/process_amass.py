...
def param_dict_for_body_model(pose_vector, trans, betas=None):
    ...
    if betas is None:
        betas = (
            torch.from_numpy(np.zeros(10)).unsqueeze(0).repeat(seqlen, 1).float().cuda()
        )
    else:
        betas = (
            torch.from_numpy(betas[:10]).unsqueeze(0).repeat(seqlen, 1).float().cuda()
        )
    ...
    body_params["left_hand_pose"] = (
        torch.from_numpy(pose_vector[:, 66:111]).float().cuda()
    )
    body_params["right_hand_pose"] = (
        torch.from_numpy(pose_vector[:, 111:]).float().cuda()
    )
...
def process_sequence(filename, use_betas, gender):
    ...
    if filename.find("BMLhandball") >= 0:
        sequence_fps = 240
    if filename.find("20160930_50032") >= 0 or filename.find("20161014_50033") >= 0:
        sequence_fps = 59
    ...
    if OUT_FPS > sequence_fps:
        ...
        downsample_ids = np.linspace(0, num_frames - 1, num=new_num_frames, dtype=int)
    ...
    if use_betas:
        ...
        body_params = param_dict_for_body_model(
            final_seq_data["poses"],
            final_seq_data["trans"],
            betas=final_seq_data["betas"],
        )
    else:
        body_params = param_dict_for_body_model(
            final_seq_data["poses"], final_seq_data["trans"]
        )
...
def read_data(input_dir, model_type, output_dir, use_betas, gender):
    amass_subsets = [
        f"{input_dir}/{x}" for x in AMASS_DIRS if os.path.isdir(f"{input_dir}/{x}")
    ]
    ...
    for sset in amass_subsets:
        ...
        if os.path.isdir(out_dir) and any(os.scandir(out_dir)):
            print(
                f"The directory: {out_dir}/{dataset_name} "
        ...
        for seq in tqdm(seqs):
            # read data
            if (
                os.path.basename(seq) == "neutral_stagei.npz"
                or os.path.basename(seq) == "shape.npz"
            ):
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gender",
        required=True,
        choices=["male", "female", "neutral", "amass"],
        type=str,
        help="hard-code the gender or use amass gender",
    )
