...
import logging
import joblib
from mgpt.constants import PRECOMPUTED_DIR
...
def get_babel_keys(path: str):
    filepath = Path(path) / "../babel-teach/id2fname/amass-path2babel.json"
...
def process_actions(segments: List[List]):
    ...
    for i in range(len(segments) - 1):  # --> make them consecutive without overlaps
        ...
        segments[i], segments[i + 1] = (
            (segments[i][0], segments[i][1] + over // 2),
            (segments[i][1] + over // 2 + 1, segments[i + 1][1]),
        )

    # check all are consecutive
    for i in range(len(segments) - 1):
        assert (
            segments[i][1] + 1 == segments[i + 1][0]
        ), f"{segments[i][1] + 1} != {segments[i+1][0]}"
    return segments
        ...
        if babel_labels["frame_ann"] is None:  # single sequence/action
            ...
            for seg_, ov_seg in overlaps_for_each_seg.items():
                for seg2 in ov_seg["before"]:
                    assert (
                        seg_ in overlaps_for_each_seg[seg2]["after"]
                    ), f"{seg_} not in {seg2} after. Relationship must be symmetric"
            ...
            def recursive(ov_per_seg, cur_seg, current_pair=[]):
                ...
                if (
                    ...
                    )  # finish pair because it is a transition, so (before, next) are a valid pair.
                    ...
                    cur_seg_pairs_af = (
                        list(itertools.product([cur_seg], after_noT))
                        if after_noT
                        else []
                    )
                    ...
                    if after_noT:  # if there are after segments, then we can add the current segment to the pair
                        ...
                        current_pair != [] and ov_seg["after"]
                    ):  # skip this segment, and look at the next one (transition)
                        cur_seg_pairs = []
                ...
                # just to be sure
                cur_seg_pairs = [
                    sorted(p, key=lambda item: item[0]) for p in cur_seg_pairs
                ]
                ...
                for segs in cur_seg_pairs:
                    ...
                    if segs[-1][1] - segs[0][0] > maxframes:
                        all_seg_pairs.append(
                            segs[:-1]
                        )  # do not look further for this segment, already OVERFLOWS
                    else:  # find next pair
                        all_seg_pairs += recursive(
                            ov_per_seg, segs[-1], segs
                        )  # look for the next pair, starting from the last segment of the current pair
            ...
            for seg_, ov_seg in overlaps_for_each_seg.items():
                ...
                if len(cur_seg_pairs) > 0:
                    cur_act_pairs = [
                        [seg2act[s] for s in segs] for segs in cur_seg_pairs
                    ]
...
class BABEL(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.precomputed_folder = PRECOMPUTED_DIR.joinpath("babel", "tmp", "custom")
        if not os.path.exists(self.precomputed_folder) or not os.path.exists(
            os.path.join(self.precomputed_folder, f"{split}_motion_data.pkl")
        ):
            if progress_bar:
            ...
            for i, sample in enumerator:
                ...
                for index, seg in enumerate(seg_ids):
                    fsegs = process_actions(
                        seg
                    )  # make them consecutive without overlaps.
                    ...
                    smpl_data = {
                        "poses": torch.from_numpy(
                            sample["poses"][frames]
                        ).float(),  # --> [seq_len, 156] from SMPL-H pose params
                    ...
                    valid_data_len += 1
                    if (
                        len(seg_acts[index]) == 1
                        and seg_acts[index][0] in EXCLUDED_ACTIONS
                    ):  # individual excluded sequences
                        num_bad_actions += 1
                        continue
                    ...
                    )  # .rots --> [seq_len, 22, 3, 3], trans --> [seq_len, 3]
                    features = self.transforms.rots2rfeats(
                        smpl_data
                    )  # --> [seq_len, 135]
            ...
            if self.parse_tokens:
                ...
                with open(
                    os.path.join(self.precomputed_folder, f"{split}_tokens_data.pkl"),
                    "wb",
                ) as f:
            ...
            with open(
                os.path.join(self.precomputed_folder, f"{split}_motion_data.pkl"), "wb"
            ) as f:
                pickle.dump(motion_data, f)
            with open(
                os.path.join(self.precomputed_folder, f"{split}_texts_data.pkl"), "wb"
            ) as f:
                pickle.dump(texts_data, f)
            with open(
                os.path.join(self.precomputed_folder, f"{split}_durations.pkl"), "wb"
            ) as f:
            ...
            if split == "train":
                ...
                with open(
                    os.path.join(self.precomputed_folder, f"{split}_motions_xyz.pkl"),
                    "wb",
                ) as f:
            ...
            with open(
                os.path.join(self.precomputed_folder, f"{split}_motion_data.pkl"), "rb"
            ) as f:
                motion_data = pickle.load(f)
            with open(
                os.path.join(self.precomputed_folder, f"{split}_texts_data.pkl"), "rb"
            ) as f:
                texts_data = pickle.load(f)
            with open(
                os.path.join(self.precomputed_folder, f"{split}_durations.pkl"), "rb"
            ) as f:
    ...
    def precompute_text_embeddings(self, clip_version):
        ...
        if not os.path.exists(self.precomputed_folder) or not os.path.exists(
            os.path.join(self.precomputed_folder, f"{self.split}_clip_embeddings.pkl")
        ):
            ...
            with open(
                os.path.join(
                    self.precomputed_folder, f"{self.split}_clip_embeddings.pkl"
                ),
                "wb",
            ) as f:
...
class BABEL_SingleEval(BABEL):
    def __init__(
        ...
        **kwargs,
        ...
        if not os.path.exists(self.precomputed_folder) or not os.path.exists(
            os.path.join(self.precomputed_folder, f"{split}_motion_data{suffix}.pkl")
        ):
            if progress_bar:
                enumerator = enumerate(tqdm(data_for_split, f"Loading BABEL {split}"))
            else:
            ...
            for i, sample in enumerator:
                ...
                last_framerate = sample["fps"]
                babel_id = sample["babel_id"]
                seg_ids, seg_acts, valid = extract_single_segments(
                    self.babel_annots[babel_id],
                    fps=last_framerate,
                    seqlen=nframes_total,
                )
                ...
                for index, seg in enumerate(seg_ids):
                    ...
                    if not accept and (
                        not self.crop_sample
                        or not self.sampler.can_be_cropped(duration)
                    ):
            ...
            with open(
                os.path.join(
                    self.precomputed_folder, f"{split}_motion_data{suffix}.pkl"
                ),
                "wb",
            ) as f:
                pickle.dump(motion_data, f)
            with open(
                os.path.join(
                    self.precomputed_folder, f"{split}_texts_data{suffix}.pkl"
                ),
                "wb",
            ) as f:
                pickle.dump(texts_data, f)
            with open(
                os.path.join(self.precomputed_folder, f"{split}_durations{suffix}.pkl"),
                "wb",
            ) as f:
                pickle.dump(durations, f)
    ...
    def __getitem__(self, item):
        ...
        word_embeddings, pos_one_hots, sent_len, tokens = process_tokens(
            tokens, self.opt.max_text_len, self.w_vectorizer
        )
        ...
        if not os.path.exists(self.precomputed_folder) or not os.path.exists(
            ...
            for i, sample in enumerator:
                ...
                for j in range(num_samples):
                    duration = (
                        np.random.randint(self.sampler.min_len, self.sampler.max_len)
                        if self.sampler.max_len > self.sampler.min_len
                        else self.sampler.min_len
                    )
                    t0 = (
                        np.random.randint(0, nframes_total - duration)
                        if nframes_total > duration
                        else 0
                    )
                    t1 = t0 + duration
                    ...
                    smpl_data = {
                        ...
                        "trans": torch.from_numpy(sample["trans"][t0:t1]).float(),
                    }  # --> [seq_len, 3]
