...
import numpy as np
from mgpt.data_loaders.tensors import lengths_to_mask
import os
import json
from mgpt.data_loaders.amass.babel import get_tokens
from mgpt.data_loaders.humanml.utils.get_opt import get_opt
from mgpt.data_loaders.humanml.data.dataset import process_tokens
...
class CompMDMGeneratedDataset(Dataset):
        ...
        with open(eval_file, "r") as f:
            ...
            for i in range(len(all_model_kwargs)):
                ...
                if (
                    ...
                    and "scenario" in kwargs["y"]
                    and kwargs["y"]["scenario"] != scenario
                ):
                    continue  # skip this one

                kwargs["y"]["lengths"] = torch.tensor(
                    [int(v) for v in kwargs["y"]["lengths"]]
                )
                kwargs["y"]["mask"] = (
                    lengths_to_mask(
                        kwargs["y"]["lengths"], kwargs["y"]["lengths"].max()
                    )
                    .unsqueeze(1)
                    .unsqueeze(1)
                )
            ...
            assert (
                len(final_model_kwargs) > 0
            ), f"No model kwargs found for this scenario: {scenario}"
            print(
                f"loaded {len(final_model_kwargs)} model kwargs for scenario {scenario if scenario != '' else '> all <'}"
            )
    ...
    def __init__(
        self, args, model, diffusion, mm_num_samples, mm_num_repeats, eval_file
    ):
        ...
        if mm_num_samples > 0:
            mm_idxs = np.random.choice(
                len(dataloader), mm_num_samples // num_seqs + 1, replace=False
            )
    ...
    def __getitem__(self, item):
        ...
        if (
            self.dataset_name != "babel"
        ):  # babel already takes care of its de)normalization itself
            normed_motion = motion
            denormed_motion = normed_motion * self.std + self.mean
            renormed_motion = (
                denormed_motion - self.mean_for_eval
            ) / self.std_for_eval  # according to T2M norms
    ...
    def switch_target(self, target):
        assert target in [
            "motion",
        ], "Only motion eval target is available for non-unfolding dataset"
...
class CompMDMUnfoldingGeneratedDataset(CompMDMGeneratedDataset):
    def __init__(
        ...
        self.opt = opt
        assert (
            self.dataset_name == "babel" or self.dataset_name == "humanml"
        ), "Only babel and humanml are supported"
        ...
        with torch.no_grad():
            for i, model_kwargs in tqdm(dataloader):
                precomputed_file_kwargs = os.path.join(
                    precomputed_folder, f"{i:02d}_kwargs.json"
                )
                ...
                if os.path.exists(precomputed_file_kwargs):
                    ...
                    kwargs = (
                        loaded_kwargs if "y" in loaded_kwargs else {"y": loaded_kwargs}
                    )  # to keep compatibility with the old format
                    # assert equal
                    assert (
                        kwargs["y"]["lengths"] == model_kwargs["y"]["lengths"].tolist()
                    )
                    ...
                    if os.path.exists(precomputed_file_pt):
                        ...
                        unfolded = np.load(
                            precomputed_file_npy, allow_pickle=True
                        ).item()
                        rots = torch.tensor(
                            unfolded["rots"]
                        )  # --> [seq_len, 22, 3, 3], rot matrices
                        transl = torch.tensor(
                            unfolded["transl"]
                        )  # --> [seq_len, 3], translations
                        ...
                        rots = matrix_to_axis_angle(
                            rots
                        )  # --> [seq_len, 22, 3], axis angle
                    ...
                    assert (
                        model is not None and diffusion is not None
                    ), "Model and diffusion must be provided for evaluation if precomputed files are not available"
                    # compute it
                    model.eval()
                    ...
                    json.dump(
                        ...
                        open(precomputed_file_kwargs, "w"),
                    )
                ...
                for bs_i in range(num_seqs):
                        ...
                        (start + model_kwargs["y"]["lengths"][bs_i])
                        if bs_i != num_seqs - 1
                        else None
                    )
                    motion_slice = (
                        unfolded[..., start:end].squeeze().permute(1, 0).cpu().numpy()
                    )
                    assert (
                        motion_slice.shape[0] == model_kwargs["y"]["lengths"][bs_i]
                    ), f'{motion_slice.shape[0]} != {model_kwargs["y"]["lengths"][bs_i]}'

                    generated_motion.append(
                        {
                            "motion": pad_sample_with_zeros(
                                motion_slice, self.max_motion_length
                            ),
                            "length": model_kwargs["y"]["lengths"][bs_i],
                            "caption": model_kwargs["y"]["text"][bs_i],
                        }
                ...
                for bs_i in range(num_seqs - 1):  # last one has no transition
                    ...
                    motion_slice = (
                        unfolded[..., mid - l_margin : mid + r_margin]
                        .squeeze()
                        .permute(1, 0)
                        .cpu()
                        .numpy()
    ...
    def switch_target(self, target):
        ...
        self.generated_motion = (
            self.generated_inbetween
            if self.target == "motion"
            else self.generated_transitions
        )
