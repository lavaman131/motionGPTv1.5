...
from mgpt.data_loaders.amass.transforms import SMPLTransform
from mgpt.data_loaders.get_data import get_dataset_loader
from mgpt.data_loaders.humanml.options.train_options import TrainTexMotMatchOptions

from mgpt.data_loaders.humanml.networks.modules import (
    MovementConvDecoder,
    MovementConvEncoder,
    TextEncoderBiGRUCo,
    MotionEncoderBiGRUCo,
)
from mgpt.data_loaders.humanml.networks.trainers import TextMotionMatchTrainer
from mgpt.data_loaders.humanml.data.dataset import (
    HumanML3D_Text2MotionDatasetV2,
    collate_fn,
)
import numpy as np
from torch.utils.data import DataLoader
from mgpt.data_loaders.humanml.utils.word_vectorizer import (
    WordVectorizer,
    POS_enumerator,
)
...
def build_models(opt):
    movement_enc = MovementConvEncoder(
        opt.dim_pose - opt.foot_contact_entries,
...
if __name__ == "__main__":
    ...
    if opt.dataset_name == "t2m":
        ...
        meta_root = pjoin(
            opt.checkpoints_dir, opt.dataset_name, "Comp_v6_KLD01", "meta"
        )
    ...
    trainer = TextMotionMatchTrainer(
        opt, text_encoder, motion_encoder, movement_encoder
    )
    ...
    if opt.dataset_name == "babel":
        train_loader = get_dataset_loader(
            ...
            num_frames=(30, 200),  # not in use
            split="train",
            load_mode="evaluator_train",
            opt=opt,
        )
