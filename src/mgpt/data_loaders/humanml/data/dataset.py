...
from mgpt.data_loaders.humanml.utils.word_vectorizer import WordVectorizer
from mgpt.data_loaders.humanml.utils.get_opt import get_opt
import torch

from mgpt.data_loaders.amass.babel_flowmdm import (
    BABEL_SingleEval,
    BABEL_TransitionsEval,
)
...
class HumanML3D_Text2MotionDatasetV2(data.Dataset):
    def __init__(
        ...
        self.precomputed_folder = PRECOMPUTED_DIR.joinpath("HumanML3D", "tmp")
        os.makedirs(self.precomputed_folder, exist_ok=True)
        suffix = (
            f"{num_frames}"
            if num_frames == False or type(num_frames) == int
            else f"{num_frames[0]}_{num_frames[1]}"
        )
        self.split = split_file.split("/")[-1].split(".")[0]
        if not os.path.exists(self.precomputed_folder) or not os.path.exists(
            os.path.join(self.precomputed_folder, f"{self.split}_data_{suffix}.pkl")
        ):
            from mgpt.data_loaders.amass.babel_flowmdm import (
                ...
                try:
                    ...
                    with cs.open(pjoin(opt.text_dir, name + ".txt")) as f:
                        for line in f.readlines():
                            ...
                            if self.split == "train":
                                text_dict["text_embedding"] = (
                                    encode_text(clip_model, [caption], device)
                                    .cpu()
                                    .numpy()
                                )
            ...
            with open(
                os.path.join(
                    self.precomputed_folder, f"{self.split}_data_{suffix}.pkl"
                ),
                "wb",
            ) as f:
    ...
    def __getitem__(self, item):
        ...
        text_embedding = (
            text_data["text_embedding"].squeeze() if self.split == "train" else []
        )
        ...
        return (
            ...
            [],
            text_embedding,
        )  # last [] is for transition sequence, which is not used in this dataset
...
class HumanML3D(data.Dataset):
    def __init__(
        ...
        if (
            load_mode == "gt"
        ):  # GT is always used to eval GT --> from ORIGINAL UNNORMALIZED to evaluators normalization
            ...
            "eval",
            "gen",
        ]:  # from ORIGINAL UNNORMALIZED to training normalization
            # used by our models
        ...
        self.t2m_dataset = HumanML3D_Text2MotionDatasetV2(
            self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, **kwargs
        )
...
class BABEL_eval(data.Dataset):
    def __init__(
        self, load_mode, datapath, transforms, sampler, opt, split="train", **kwargs
    ):
        self.load_mode = load_mode
