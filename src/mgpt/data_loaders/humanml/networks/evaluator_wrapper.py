...
from mgpt.data_loaders.humanml.utils.word_vectorizer import POS_enumerator
...
def build_evaluators(opt):
    ...
    checkpoint = torch.load(
        pjoin(opt["checkpoints_dir"], ckpt_dir, model_name, "model", "finest.tar"),
...
class EvaluatorMDMWrapper(object):
    ...
    def get_co_embeddings(self, word_embs, pos_ohot, cap_lens, motions, m_lens):
        with torch.no_grad():
            ...
            """Movement Encoding"""
            movements = self.movement_encoder(
                motion_wo_foot_contact(motions, self.opt["foot_contact_entries"])
            ).detach()
