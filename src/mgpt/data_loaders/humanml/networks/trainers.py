...
import random
from mgpt.data_loaders.humanml.networks.modules import ContrastiveLoss
...
from mgpt.data_loaders.humanml.utils.utils import print_current_loss_decomp
from os.path import join as pjoin
from mgpt.data_loaders.humanml.data.dataset import collate_fn
import codecs as cs
from mgpt.utils.train_platforms import (
    ClearmlPlatform,
    TensorboardPlatform,
    NoPlatform,
...
class Logger2(object):
    ...
    def scalar_summary(self, tag, value, step):
        self.train_platform.report_scalar(
            name=tag, value=value, iteration=step, group_name="Loss"
        )
            ...
            self.mov_enc(
                torch.zeros(
                    (
                        ...
                        self.motions.shape[-1] - self.opt.foot_contact_entries,
                    ),
                    device=self.device,
                )
            )
            ...
            self.mov_enc(
                torch.zeros(
                    (
                        word_emb.shape[0],
                        self.opt.unit_length,
                        dim_pose - self.opt.foot_contact_entries,
                    ),
                    device=self.device,
                )
            )
