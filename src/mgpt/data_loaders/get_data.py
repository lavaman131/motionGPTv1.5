...
from mgpt.data_loaders.tensors import collate as all_collate
from mgpt.data_loaders.tensors import t2m_collate
from mgpt.constants import BABEL_DATA_DIR


def get_dataset_class(name, load_mode):
    if name == "babel":
        if (
            "gt" in load_mode or load_mode == "evaluator_train"
        ):  # reference motion for evaluation
            from mgpt.data_loaders.humanml.data.dataset import BABEL_eval

            return BABEL_eval
        elif load_mode == "gen":
            from mgpt.data_loaders.amass.babel import BABEL

            return BABEL
        elif load_mode == "train":
            from mgpt.data_loaders.amass.babel_flowmdm import BABEL

            return BABEL
        else:
            raise ValueError(f"Unsupported load_mode name [{load_mode}]")
    elif name == "humanml":
        from mgpt.data_loaders.humanml.data.dataset import HumanML3D
...
def get_collate_fn(name, load_mode="train"):
    ...
    if "gt" in load_mode:
        from mgpt.data_loaders.humanml.data.dataset import (
    ...
    if name in ["humanml"]:
        ...
        from mgpt.data_loaders.tensors import babel_collate

        return babel_collate
    elif name == "babel":
        from mgpt.data_loaders.humanml.data.dataset import collate_fn as sorted_collate
...
def get_dataset(name, num_frames, split="train", load_mode="train", **kwargs):
    ...
    if name in ["humanml"]:
        ...
        from mgpt.data_loaders.amass.sampling import FrameSampler

        if split == "val" and cropping_sampler:
        ...
        dataset = DATA(
            split=split,
            datapath=str(BABEL_DATA_DIR.joinpath("babel-smplh-30fps-male")),
...
def get_dataset_loader(
    ...
    **kwargs,
    ...
    dataset = get_dataset(
        name, num_frames, split, load_mode, batch_size=batch_size, **kwargs
    )
