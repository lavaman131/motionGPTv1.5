...
from mgpt.model.x_transformers.attend import Attend, Intermediates, CascadingHeads
...
class BidirectionalAlibiPositionalBias(AlibiPositionalBias):
    def __init__(self, heads, total_heads, **kwargs):
        ...
        self.heads_noncausal = (
            heads // 2
        )  # if odd, there is one more head causal than noncausal
    ...
    def forward(self, i, j):
        ...
        # for efficiency
        if (
            exists(self.bias) and self.bias.shape[-1] >= j and self.bias.shape[-2] >= i
        ):  # if previous adequate bias is buffered
            return self.bias[..., :i, :j]
        ...
        noncausal_mask = (
            torch.tril(neg_inf, -1).unsqueeze(0).repeat(self.heads_noncausal, 1, 1)
        )
...
class AbsoluteAlibiPositionalBias(AlibiPositionalBias):
    def forward(self, i, j):
        ...
        bias = (
            -torch.arange(j, device=self.device).unsqueeze(0).unsqueeze(0) / j
        )  # from 0 to 1
...
class BPE_Rotary(nn.Module):
    def __init__(self, schedule, *args, **kwargs):
        ...
        assert hasattr(
            self.schedule, "get_time_weights"
        ), "schedule needs to implement method 'get_time_weights'"
    ...
    def forward(self, seq_len, device, timesteps, training, pe_bias, **kwargs):
        ...
        if training or (
            not exists(self.freqs_rel) or self.last_timestep < timesteps[0]
        ):
        ...
        w = (
            self.schedule.get_time_weights(timesteps, training)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        ...
        if pe_bias is not None:
            assert (
                w.int() == w
            ).all(), "w should be 0 or 1 when using multitext at training"
            pe_bias[w.squeeze() == 1] = (
                0  # need to zero bias out for the relative PE batch elements
            )
...
class BPE_AbsoluteSinusoidal(nn.Module):
    ...
    def _forward_multisegment(
        self, seq_len, device, timesteps, pos_pe_abs, training, pe_bias, **kwargs
    ):
        # on inference --> every time we start denoising a batch, we recompute freqs_abs and rel adapted to that max seq length.
        # on training --> always recompute (each length of sequence needs different absolute PE)
        if training or (
            not exists(self.freqs_abs_multi) or self.last_timestep < timesteps[0]
        ):
            assert timesteps is not None
            freqs_abs = self.absolute.forward_seqlen(seq_len, device)

            freqs_abs_rep = freqs_abs.expand(
                pos_pe_abs.shape[0], -1, -1
            )  # add batch dimension
            sel_freqs_abs = torch.gather(
                freqs_abs_rep,
                dim=1,
                index=pos_pe_abs.to(int).unsqueeze(-1).expand(-1, -1, self.ape_dim),
            )  # select according to those indices
    ...
    def forward(self, seq_len, device, timesteps, pos_pe_abs=None, **kwargs):
        if (
            pos_pe_abs is not None
        ):  # (init, end) inside segments:=[batch_size, 2] are the boundaries of each individual segment. If None, the whole sequence is considered as a single segment
            return self._forward_multisegment(
                seq_len, device, timesteps, pos_pe_abs, **kwargs
            )
        else:
...
class Attention(nn.Module):
    def __init__(
        ...
        self.heads = heads
        self.causal = causal
        self.max_attend_past = (
            max_attend_past
            if max_attend_past is not None and max_attend_past >= 0
            else None
        )
        self.max_attend_future = (
            max_attend_future
            if max_attend_future is not None and max_attend_future >= 0
            else None
        )
    ...
    def forward(
        ...
        if exists(self.custom_query_fn):
            assert exists(
                cond_tokens
            ), "conditional tokens must be passed in if using a custom query function"
            q_input = self.custom_query_fn(
                torch.cat([x, cond_tokens], axis=-1)
        ...
        if exists(rotary_pos_emb):  # and not has_context:
            ...
            )  # split query, key, value into rotary and non-rotary parts
            ...
            )  # concat query, key, value rotary and non-rotary parts
        ...
        if exists(self.max_attend_past) or exists(self.max_attend_future):
            ...
            if exists(self.max_attend_future):
                max_attend_future_mask = (
                    dist < -self.max_attend_future
                )  # indices in the future are negative
        ...
        if exists(rel_pos):
            if exists(attn_bias) and len(attn_bias.shape) == 2:
                attn_bias = attn_bias.unsqueeze(
                    0
                )  # head dimension is needed for relative positional bias
            attn_bias = (
                attn_bias + rel_pos(i, j) if exists(attn_bias) else rel_pos(i, j)
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if rotary_bpe_schedule is None and rotary_pos_emb:
            ...
            self.rotary_pos_emb = BPE_Rotary(
                rotary_bpe_schedule,
                rotary_emb_dim,
                use_xpos=rotary_xpos,
                scale_base=rotary_xpos_scale_base,
        ...
        self.num_attn_layers = len(
            list(filter(equals("a"), [l.lower() for l in layer_types]))
        )
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(
            ...
            if layer_type in ["a", "A"]:
                assert (
                    layer_type == "a" or custom_query_fn is not None
                ), "custom query function must be passed in if using cross attention A"
                layer = Attention(
                    dim,
                    heads=heads,
                    causal=causal,
                    custom_query_fn=custom_query_fn[a_idx]
                    if isinstance(custom_query_fn, (list, tuple))
                    else custom_query_fn,
                    **attn_kwargs,
                )
    ...
    def forward(
        ...
        if exists(self.rotary_pos_emb):
            ...
            rotary_pos_emb = self.rotary_pos_emb(
                max_rotary_emb_length, x.device, **rotary_kwargs
            )
        ...
        for ind, (layer_type, (norm, block, residual_fn), layer_dropout) in enumerate(
            ...
            if layer_type in ["a", "A"]:
                ...
                _x, _context = (
                    (x, context) if layer_type == "c" else (context, x)
                )  # else is custom crossattention
                out, inter = block(
                    _x,
                    context=_context,
                    mask=mask,
    ...
    def __init__(
        ...
        *,
        ...
        if not (use_abs_pos_emb):  # and not attn_layers.has_pos_emb):
            ...
            assert (
                absolute_bpe_schedule is not None
            ), "mixed ape schedule must be passed in if using mixed ape"
