...
from mgpt.model.rotation2xyz import Rotation2xyz
from mgpt.model.MDM import InputProcess, OutputProcess
from mgpt.model.base_models import TextConditionalModel
from mgpt.model.x_transformers.x_transformers import (
...
class FlowMDM(TextConditionalModel):
    def __init__(
        ...
        **kargs,
        ...
        bpe_training_rate = kargs.get(
            "bpe_training_ratio", 0.5
        )  # for training, we dropout with prob 50% --> APE vs RPE
        bpe_inference_step = kargs.get("bpe_denoising_step", None)
        diffusion_steps = kargs.get("diffusion_steps", None)
        self.bpe_schedule = BPE_Schedule(
            bpe_training_rate, bpe_inference_step, diffusion_steps
        )
        ...
        print(
            f"[Inference] BPE switch from APE to RPE at denoising step {bpe_inference_step}/{diffusion_steps}."
        )
        ...
        self.seqTransEncoder = ContinuousTransformerWrapper(
            ...
            attn_layers=Encoder(
                ...
                ff_mult=int(
                    np.round(self.ff_size / self.latent_dim)
                ),  # 2 for MDM hyper params
    ...
    def forward(self, x, timesteps, y):
        ...
        mask = (
            y["mask"].reshape((bs, nframes))[:, :nframes].to(x.device)
        ).bool()  # [bs, max_frames]

        self.bpe_schedule.step(
            timesteps, self.training
        )  # update the BPE scheduler (decides either APE or RPE for each timestep)
        if self.training or self.bpe_schedule.use_bias(timesteps, self.training):
            pe_bias = y.get(
                "pe_bias", None
            )  # This is for limiting the attention to inside each conditioned subsequence. The BPE will decide if we use it or not depending on the dropout at training time.
            chunked_attn = False
        ...
        rotary_kwargs = {
            "timesteps": timesteps,
            "pos_pe_abs": y.get("pos_pe_abs", None),
