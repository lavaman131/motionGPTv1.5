...
from mgpt.model.MDM import PositionalEncoding, TimestepEmbedder
...
class TextConditionalModel(nn.Module):
        ...
        if cond_mode != "no_cond":
            if "text" in cond_mode:
                ...
                raise NotImplementedError(
                    "only conditioning with text is implemented for now"
                )
    ...
    def encode_text(self, raw_text):
        ...
        max_text_len = (
            20 if self.dataset in ["humanml"] else None
    ...
    def compute_embedding(self, x, timesteps, y):
        ...
        if not force_mask:
            if "text" == self.cond_mode:
                primitive = (
                    ...
                    else frozenset((frozenset(txts) for txts in y[key]))
                )
            else:
                raise ValueError
            ...
            recompute = (
                not hasattr(self, "emb_forcemask_hash")
                or self.emb_forcemask_hash != hash_value
            )
            if not recompute:
        ...
        if (
            not multitext_mode
        ):  # --> single text training (e.g. HumanML3D dataset) / inference
            enc_text = (
                self.encode_text(y["text"])
                if "text_embeddings" not in y
                else y["text_embeddings"]
            )  # if precomputed --> faster
            ...
            conditions_mask = conditions_mask.unsqueeze(-1).expand(
                -1, -1, -1, self.latent_dim
            )  # [I, T, N, d]
            cond_emb = torch.zeros(
                conditions_mask.shape[1:], device=embedded_text.device
            )  # [T, N, d]
            for i in range(I):
                m = conditions_mask[i]  # [T, N, d]
                cond_emb = cond_emb + m * embedded_text[i].unsqueeze(
                    0
                )  # [T, N, d] --> [T, N, d]
        ...
        if force_mask:
            ...
            self.register_buffer(
                "emb_forcemask_hash",
                torch.tensor(hash(frozenset(x.shape))),
                persistent=False,
            )
        else:
            self.register_buffer("emb", cond_emb, persistent=False)
            self.register_buffer(
                "emb_hash", torch.tensor(hash(primitive)), persistent=False
            )
