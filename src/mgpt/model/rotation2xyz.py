...
from mgpt.model.smpl import SMPL, JOINTSTYPE_ROOT, SMPLX
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not self.include_hands and x.shape[1] == 23 + int(translation) + int(glob):
            print(
                "Warning: you are using a model without hands, but you are passing hand data (> 23 joints)."
            )
        elif self.include_hands and x.shape[1] == 53 + int(translation) + int(glob):
            print(
                "Warning: you are using a model with hands, but you are not passing hand data (< 53 joints)."
            )
        ...
        if translation:
            x_translations = x[
                :, -1, :3
            ]  # last position in joints dimension is translation
            x_rotations = x[:, :-1]
        else:
            x_rotations = x
        ...
        if self.include_hands:
            ...
            out = self.smpl_model(
                ...
                betas=betas,
                left_hand_pose=lh_rotations,
                right_hand_pose=rh_rotations,
            )
        else:
            out = self.smpl_model(
