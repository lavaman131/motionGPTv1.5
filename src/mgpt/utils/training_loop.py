...
from mgpt.diffusion.fp16_util import MixedPrecisionTrainer
from mgpt.utils import dist_util
from mgpt.diffusion.resample import LossAwareSampler, UniformSampler
from tqdm import tqdm
from mgpt.diffusion.resample import create_named_schedule_sampler
...
import mgpt.evaluation.evaluate
...
class TrainLoop:
            ...
            for motion, cond in tqdm(self.data):
                ...
                if self.step % self.log_interval == 0:
                    data_to_report = {
                        ...
                        if k not in ["loss", "step", "samples"] and "_q" not in k
                    }
                    self.train_platform.report_data(
                        data=data_to_report, iteration=self.step, group_name="Loss"
                    )
    ...
    def evaluate(self):
        ...
        if self.eval_wrapper is not None:
            ...
            eval_dict = mgpt.evaluation.evaluate.evaluation(
                self.eval_wrapper,
                self.eval_gt_data,
                self.eval_data,
                log_file,
