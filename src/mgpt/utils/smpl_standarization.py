...
def preprocess_smplh(data, pose_rep):
    ...
    toconcat = [
        ...
        "right_hand_pose",
        "transl",
    ]  # to keep the order for rotation2xyz.py.
        ...
        if pose_rep == "rot6d":
            ...
            if k == "transl":
                ...
                data[k] = np.concatenate(
                    [data[k], np.zeros((data[k].shape[0], 3))], axis=1
                )[:, np.newaxis, :]  # motion -> (n_frames, 1, 3/6)
            else:
                aux = data[k].reshape(data[k].shape[0], -1, 3)
                data[k] = axis_angle_to_rotation_6d(
                    torch.tensor(aux)
                ).numpy()  # motion -> (n_frames, n_components/joints, 3/6)
    ...
    motion = np.concatenate(
        [data[i] for i in toconcat], axis=1
    )  # motion -> (n_frames, n_components/joints, 3/6)
    return motion
