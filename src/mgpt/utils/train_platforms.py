...
class WandbPlatform(TrainPlatform):
    def __init__(self, save_dir, project, entity):
    ...
    def report_scalar(self, name, value, iteration, group_name):
        wandb.log(
            data={name: value}, step=iteration, commit=True
        )  # iteration != self.last_committed_iter)

    def report_data(self, data, iteration, group_name):
        # data = {name: value}
        wandb.log(
            data=data, step=iteration, commit=True
        )  # iteration != self.last_committed_iter)
