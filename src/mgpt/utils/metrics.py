    ...
    for model_name, motion_loader in motion_loaders.items():
        ...
        for idx, batch in enumerate(motion_loader):  # tqdm(enumerate(motion_loader)):
            ...
            if motions.shape[-1] == 263:  # HUMANML3D ============
                ...
                motions = motion_loader.dataset.inv_transform(
                    motions
                )  # we need to recover the original denormed values.
                motions = recover_from_ric(
                    motions.float(), n_joints
                )  # --> [bs, seqlen, njoints, 3]
                ...
                motions = motions.reshape(
                    lengths.shape[0], -1, motions.shape[-2], motions.shape[-1]
                )  # --> [SEQ, 22, 3]
            else:
                raise ValueError(f"Unsupported motion loader [{model_name}]")

            batch_jerk = calculate_jerk(
                motions.cpu().numpy(), lengths.cpu().numpy()
            )  # --> [BS, SEQ]
            all_jerks.append(batch_jerk)

        all_jerks = np.concatenate(all_jerks, axis=0)  # --> [BS, SEQ]
        seq_jerks = all_jerks.mean(
            axis=0
        )  # --> [SEQ] --> mean jerk per frame in the seq
        ...
        jerk_score_dict[model_name] = (
            seq_jerks.max()
        )  # Jerk --> max jerk along the sequence

        print(
            f"---> [{model_name}] PeakJerk: {jerk_score_dict[model_name]:.4f} AUJ: {auj_score_dict[model_name]:.4f}"
        )
...
def evaluate_matching_score(
    eval_wrapper, motion_loaders, file, compute_precision=True, compute_matching=True
):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                word_embeddings, pos_one_hots, captions, sent_lens, motions, m_lens = (
                    batch[:6]
                )
                ...
                if compute_precision:
                    unique_idces = np.unique(captions, return_index=True)[
                        1
                    ]  # remove duplicate captions in the top-k calculation
                    argsmax = np.argsort(
                        dist_mat[unique_idces][:, unique_idces], axis=1
                    )
                    top_k_mat = calculate_top_k(argsmax, top_k=3)
                    corrector = (
                        text_embeddings.shape[0] / top_k_mat.shape[0]
                    )  # correct for the duplicate captions
