...
parser = argparse.ArgumentParser(description="Render a SMPL video by a j3ds npy file.")
parser.add_argument("--input", type=str, default="", help="the npy file path")
parser.add_argument("--render", type=int, default=1, help="render the video if 1")
...
if args.render:
    ...
    for i in range(data.shape[0]):
        ...
        aroot[:, 1] = -aroot[:, 1]
        params = dict(
            pred_shape=np.zeros([1, 10]), pred_root=aroot, pred_pose=pose[[i]]
        )
