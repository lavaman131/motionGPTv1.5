...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=description, prog="Batch SMPL-Untangle"
    )
    parser.add_argument(
        "--param_fn",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Display the mesh during the optimization" + " process",
    )
    parser.add_argument(
        "--delay",
        type=int,
    ...
    parser.add_argument(
        "--model_folder", type=str, default="models", help="The path to the LBS model"
    )
    parser.add_argument(
        "--model_type",
        type=str,
        default="smpl",
        choices=["smpl", "smplx", "smplh"],
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="The height of the cone used to calculate the" + " distance field loss",
    )
    parser.add_argument("--lr", default=1, type=float, help="The learning rate for SGD")
    parser.add_argument(
        "--coll_loss_weight",
        default=1e-4,
        type=float,
    ...
    for idx, fn in enumerate(param_fn):
        ...
        assert "betas" in data, "No key for shape parameter in provided pickle file"
        assert (
            "global_pose" in data
    ...
    if part_segm_fn:
        ...
        filter_faces = FilterFaces(faces_segm, faces_parents)

    # Create the body model
    body = create(model_folder, batch_size=batch_size, model_type=model_type)
    ...
    pen_distance = collisions_loss.DistanceFieldPenetrationLoss(
        sigma=sigma, point2plane=point2plane, vectorized=True
    )

    mse_loss = nn.MSELoss(reduction="sum").to()
    ...
    faces_idx = face_tensor + (torch.arange(bs, dtype=torch.long) * nv)[:, None, None]
    ...
    if interactive:
        ...
        def create_mesh(vertices, faces, color=(0.3, 0.3, 0.3, 1.0), wireframe=False):
            ...
            return pyrender.Mesh.from_trimesh(tri_mesh, material=material)
        ...
        for bidx in range(np_verts.shape[0]):
            ...
            body_mesh = create_mesh(
                curr_verts, body.faces, color=(0.3, 0.3, 0.3, 0.99), wireframe=True
            )

            pose = np.eye(4)
            pose[0, 3] = bidx * 2
            scene.add(body_mesh, name="body_mesh_{:03d}".format(bidx), pose=pose)
    ...
    while True:
        ...
        if print_timings:
            start = time.time()
        pen_loss = coll_loss_weight * pen_distance(triangles, collision_idxs)
        if print_timings:
            torch.cuda.synchronize()
            print("Penetration loss: {:5f}".format(time.time() - start))

        shape_reg_loss = torch.tensor(0, dtype=torch.float32)
        if shape_reg_weight > 0:
            shape_reg_loss = shape_reg_weight * torch.sum(body.betas**2)
        pose_reg_loss = torch.tensor(0, dtype=torch.float32)
        if pose_reg_weight > 0:
            pose_reg_loss = pose_reg_weight * mse_loss(body.pose, init_pose)
        ...
        if interactive:
            ...
            for bidx in range(batch_size):
                ...
                if len(intr_faces) > 0:
                    intr_mesh = create_mesh(
                        curr_verts, intr_faces, color=(0.9, 0.0, 0.0, 1.0)
                    )
                    scene.add(
                        intr_mesh, name="intr_mesh_{:03d}".format(bidx), pose=pose
                    )

                if len(recv_faces) > 0:
                    recv_mesh = create_mesh(
                        curr_verts, recv_faces, color=(0.0, 0.9, 0.0, 1.0)
                    )
                    scene.add(
                        recv_mesh, name="recv_mesh_{:03d}".format(bidx), pose=pose
                    )
