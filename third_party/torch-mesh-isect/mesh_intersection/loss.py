...
def calc_circumcircle(triangles, edge_cross_prod, idx=None):
    ...
    # Should be BxF
    circumradius = (
        torch.norm(alpha - beta, dim=2, keepdim=True)
...
def repulsion_intensity(x, sigma=0.5, penalize_outside=True, linear_max=1000):
    ...
    return linear_region_mask * (-x + 1 - sigma) + quad_region_mask * quad_penalty


def dist_to_cone_axis(
    points_rel,
    dot_prod,
    cone_axis,
    cone_radius,
    ...
    if vectorized:
        ...
        numerator = torch.norm(
            points_rel - dot_prod.unsqueeze(dim=-1) * cone_axis.unsqueeze(dim=-2),
            p=2,
            dim=-1,
        )
        ...
        batch_size, num_collisions = cone_radius.shape[:2]
        numerator = torch.norm(
            points_rel - dot_prod.unsqueeze(-1) * cone_axis, p=2, dim=-1
        )
        denominator = -cone_radius.view(
            batch_size, num_collisions
        ) / sigma * dot_prod + cone_radius.view(batch_size, num_collisions)

    return numerator / (denominator + epsilon)


def conical_distance_field(
    triangle_points,
    cone_center,
    cone_radius,
    cone_axis,
    ...
    if vectorized:
        ...
        axis_dist = dist_to_cone_axis(
            points_rel, dot_prod, cone_axis, cone_radius, sigma=sigma, vectorized=True
        )
        ...
        distance_field = torch.zeros(
            [batch_size, num_collisions, 3],
        ...
        for idx in range(num_points):
            ...
            distance_field[:, :, idx] = (1 - axis_dist) * mask * circumcenter_dist
...
class DistanceFieldPenetrationLoss(nn.Module):
    ...
    def forward(self, triangles, collision_idxs):
        ...
        receiver_loss = torch.tensor(0, device=triangles.device, dtype=torch.float32)
        intruder_loss = torch.tensor(0, device=triangles.device, dtype=torch.float32)
        ...
        if self.point2plane:
            ...
            receiver_loss = (
                torch.norm(
                    -phi_receivers.unsqueeze(dim=-1) * intr_normals.unsqueeze(dim=-2),
                    p=2,
                    dim=-1,
                )
                .pow(2)
            ...
            intruder_loss = (
                torch.norm(
                    -phi_intruders.unsqueeze(dim=-1) * recv_normals.unsqueeze(dim=-2),
                    p=2,
                    dim=-1,
                )
                .pow(2)
        ...
        batch_ind = torch.arange(
            0, batch_size, dtype=batch_idxs.dtype, device=triangles.device
        ).unsqueeze(dim=1)
        batch_mask = (
            batch_ind.repeat([1, num_collisions]).eq(batch_idxs).to(receiver_loss.dtype)
        )

        loss = torch.matmul(batch_mask, receiver_loss) + torch.matmul(
            batch_mask, intruder_loss
        )
        return loss
