...
class FilterFaces(nn.Module):
    def __init__(self, faces_parents=None, faces_segm=None, ign_part_pairs=None):
        super().__init__()

        if faces_parents is not None:
            faces_parents_tensor = torch.tensor(faces_parents, dtype=torch.long)
            self.register_buffer("faces_parents", faces_parents_tensor)
        else:
            self.faces_parents = None

        if faces_segm is not None:
            faces_segm_tensor = torch.tensor(faces_segm, dtype=torch.long)
        ...
        if ign_part_pairs is not None:
            pairs = map(lambda x: list(map(int, x.split(","))), ign_part_pairs)
            pairs = np.asarray(list(pairs), dtype=np.int64)

            self.extra_ign_pairs = True
            self.register_buffer("extra_pairs", torch.tensor(pairs, dtype=torch.long))
    ...
    def forward(self, collision_idxs):
        ...
        if self.faces_segm is not None:
            ...
            same_part_mask = recv_segm.eq(intr_segm).ge(1).to(collision_idxs.dtype)
        else:
        ...
        if self.faces_parents is not None:
            ...
            kintree_mask = (
                (recv_segm.eq(intr_parents) + intr_segm.eq(recv_parents))
                .ge(1)
        ...
        # ignored
        mask = (
            (kintree_mask + same_part_mask).ge(1) * collision_idxs[:, :, 0].ge(0)
        ).to(collision_idxs.dtype)
        ...
        if self.extra_ign_pairs and self.faces_segm is not None:
            mask1 = (
                (
                    ...
                    * intr_segm.unsqueeze(dim=-1).eq(self.extra_pairs[:, 1])
                )
                .ge(1)
            ...
            mask2 = (
                (
                    ...
                    * intr_segm.unsqueeze(dim=-1).eq(self.extra_pairs[:, 0])
                )
                .ge(1)
                .sum(dim=-1, keepdim=True)
            )
